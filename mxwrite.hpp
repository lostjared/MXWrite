...
namespace mx {
     ...
     * tune:   x264 tune — empty string (none), film, animation, grain,
     *         stillimage, psnr, ssim, fastdecode, zerolatency.
     ...
     *         software), "nvenc" (force resolution-selected NVENC),
     *         "h264_nvenc", or "hevc_nvenc". NVENC requests fall back to the
     *         matching software codec.
     * realtime: when true, applies low-latency settings (tune=zerolatency for
     *           x264, tune=ll + zerolatency=1 for NVENC). Overrides tune value.
     * block_when_full: when true, producer threads block if the encoder queue
     *                  is full instead of dropping frames.
     */
    struct EncodeOptions {
        ...
        std::string codec =
            "auto"; ///< Encoder selection policy or concrete NVENC codec.
        bool realtime = false; ///< Enable low-latency settings.
        bool block_when_full =
            false; ///< Block producer threads instead of dropping when the
                   ///< encoder queue is full.

        /**
         * @brief HDR output options.
         *
         * When @ref HdrInfo::enabled is true, the writer switches to a
         * dedicated HEVC Main10 + BT.2020 output path that:
         *  - Encodes with libx265 at 10-bit (AV_PIX_FMT_YUV420P10LE).
         *  - Tags the stream with BT.2020 primaries, BT.2020 non-constant
         * luminance matrix, and SMPTE ST.2084 (PQ) transfer.
         *  - Converts incoming 8-bit sRGB RGBA shader output into PQ-encoded
         *    10-bit YUV, placing SDR-range content at the 100-nit reference
         * level inside the PQ signal (SDR-in-HDR-container).
         *  - Copies @ref mastering_display and @ref content_light side data
         * from the input stream when provided, so player HDR metadata is
         * preserved.
         *
         * This mode is intended for use when the *input* video is HDR; the
         * 8-bit GL pipeline cannot reconstruct the original highlight
         * precision, but the resulting file is a correctly-tagged HDR
         * container.
         */
        struct HdrInfo {
    ...
    class Writer {
      public:
        ...
        /**
         * @brief Write a 16-bit RGBA frame that is already PQ- or HLG-encoded
         * in BT.2020 primaries (8 bytes/pixel: R16,G16,B16,A16, little-endian
         *        unsigned normalised).
         *
        ...
        /**
         * @brief Open a timestamp-based output stream using the legacy CRF
         * string interface.
         * @param filename Output file path.
         * @param width Output width in pixels.
         * @param height Output height in pixels.
         ...
         * @return true on success.
         */
        bool open_ts(const std::string &filename, int width, int height,
                     float fps, const char *crf);
        /**
        ...
        /// @brief If true, producer threads block when the encoder queue is
        /// full instead of dropping frames. Intended for headless/batch
      ...
      private:
        ...
        double last_duration =
            0.0; ///< Cached duration from the last encode step.
...
#ifdef MXWRITE_HAS_CUDA_COPY
        // Dedicated stream so the producer's RGBA→hwframe copy does not
        // serialise with the renderer's default-stream work or with the encoder
        // thread.
        cudaStream_t cuda_upload_stream = nullptr;
#endif
        bool hdr_output =
            false; ///< True when HDR (HEVC Main10/PQ) output is active.
        EncodeOptions::HdrInfo
            hdr_info; ///< HDR metadata captured at open() time.
        SwsContext *sws_ctx = nullptr; ///< Frame conversion context.
        AVRational time_base;          ///< Stream time base.
        /** @brief Convert a frame rate into a rational numerator/denominator
         * pair. */
        ...
        // Memory cost is bounded by the NVENC frame pool / sw RGBA frame
        // buffer.
        ...
        /** @brief Initialize CUDA/NVENC resources when hardware encoding is
         * selected. */
